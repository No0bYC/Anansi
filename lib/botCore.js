...
// Supabase/PostgREST plafonne les résultats à 1000 lignes par défaut — sans
// pagination, tout contact au-delà de ce seuil devient invisible pour les
// endpoints qui font un simple select("*"). Cette fonction récupère TOUT,
// exactement comme le fait déjà loadContacts() côté frontend.
async function fetchAllContacts(supabase) {
  const PAGE = 1000;
  let all = [];
  let from = 0;
  while (true) {
    const { data, error } = await supabase.from("contacts").select("*").range(from, from + PAGE - 1);
