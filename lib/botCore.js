...
async function extractInterviewAnswer(questionAsked, answerText, contact) {
  ...
  const system =
    ...
    "Sa réponse peut contenir PLUS d'informations que ce qui était strictement demandé — capture TOUT ce qu'il mentionne " +
    "(relation, poste, entreprise, secteur, hobbies, etc.), pas seulement la réponse à la question posée. " +
...
async function processImageMessage(base64Data, mediaType, captionText, channel) {
  ...
  try {
    ...
    if (matches.length && (matches.length === 1 || matches[0].score - (matches[1] ? matches[1].score : 0) > 0.15)) {
      ...
      const patch = buildPatchFromFields(contact, cardFields);
      ...
      if (isStopCommand(text)) {
        ...
        if (contact) {
          ...
          try {
            ...
            patch = buildPatchFromFields(contact, extractedFields);
          } catch (e) {
