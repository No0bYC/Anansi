...
// Supabase/PostgREST plafonne les résultats à 1000 lignes par défaut, ET
// sans ORDER BY explicite, l'ordre des lignes n'est PAS garanti stable entre
// deux appels — un simple select("*") tronque silencieusement, et paginer
// sans tri peut faire "tomber" des lignes entre deux pages. Cette fonction
// unique (utilisée PARTOUT dans le bot, avec ou sans client passé en
// paramètre) corrige les deux problèmes à la fois.
async function fetchAllContacts(supabase) {
  ...
  let from = 0;
  while (true) {
    const { data, error } = await sb.from("contacts").select("*").order("created_at", { ascending: false }).range(from, from + PAGE - 1);
...
// Choisit UNE question de suivi ciblée sur le contact qu'on vient de créer/modifier,
// centrée sur psyché, métriques, connexions et groupes — pour enrichir la fiche
// tout de suite après une confirmation, sans dériver vers un autre contact.
function pickPostSaveFollowUp(contact, isNewContact) {
  const name = ((contact.first_name || "") + " " + (contact.last_name || "")).trim() || "cette personne";
  if (!(contact.primary_lever || "").trim()) {
    return { field: "primary_lever", question: "Pour compléter le profil de " + name + " : qu'est-ce qui compte le plus pour elle/lui ? (statut, réciprocité, appartenance, intérêt, cohérence)" };
  }
  if (!(contact.ego_type || "").trim()) {
    return { field: "ego_type", question: name + " est plutôt du genre à vouloir faire des choses, en avoir, ou être perçu d'une certaine façon ?" };
  }
  if (!(contact.groups || []).length) {
    return { field: "groups_add", question: name + " fait partie d'un groupe ou cercle particulier chez toi ? (famille, sport, business, spiritualité, hobby...)" };
  }
  if (!(contact.connections || []).length) {
    return { field: "mentioned_contacts", question: name + " est en lien avec d'autres personnes de ta base ? (collègues, amis communs, famille...)" };
  }
  if (isNewContact && (contact.utility_score ?? 5) === 5 && (contact.sentiment_score ?? 5) === 5 && (contact.reliability_score ?? 5) === 5) {
    return { field: "metrics", question: "Pour finir : sur 10, à quel point " + name + " est utile pour tes objectifs, à quel point tu l'apprécies, et à quel point tu lui fais confiance ?" };
  }
  return null;
}
...
const INTERVIEW_ANSWER_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      utility_score: { type: "number", description: "0 à 10 — utilité de ce contact pour les objectifs de Yann, si mentionné ou clairement déductible." },
      sentiment_score: { type: "number", description: "0 à 10 — appréciation personnelle de Yann envers ce contact, si mentionné ou clairement déductible." },
      reliability_score: { type: "number", description: "0 à 10 — niveau de confiance de Yann envers ce contact, si mentionné ou clairement déductible." },
      mentioned_contacts: {
        type: "array",
...
async function answerQuestion(questionText, contacts) {
  const summary = contacts.map((c) => {
    const parts = [
      ...
      c.region ? "région: " + c.region : "",
      c.country ? "pays: " + c.country : "",
  ...
  const system =
    ...
    "Pour une question du type \"qui puis-je contacter à/en [pays/ville] pour [besoin]\" : croise pays, région, ville, " +
    "origine ET secteur/rôle pour trouver les meilleurs candidats — quelqu'un dont l'ORIGINE correspond à ce pays peut " +
    "être pertinent même s'il vit ailleurs aujourd'hui (son réseau/influence peut s'étendre jusque là-bas). " +
    "Donne le nom, le lien avec ce pays (résidence ou origine), et les coordonnées disponibles (téléphone/email) pour " +
    "que Yann puisse le contacter directement. Classe par pertinence si plusieurs candidats. " +
...
// ═══════════════════════════════════════════════════════════════════════════
// VEILLE CONTINUE — recherche d'actualité hebdomadaire ou à la demande
// ═══════════════════════════════════════════════════════════════════════════
const MONITORING_CRITERIA = ["Business", "Politique", "Social", "Influence", "Art", "Sport"];

const MONITORING_TOOL = {
  name: "structure_monitoring_report",
  description: "Structure les résultats d'une recherche de veille hebdomadaire sur une personne en rapport exploitable.",
  input_schema: {
    type: "object",
    properties: {
      found_anything: { type: "boolean", description: "false si aucune actualité récente pertinente n'a été trouvée." },
      summary: { type: "string", description: "Synthèse en 2-3 phrases de ce qui s'est passé récemment pour cette personne, selon les critères demandés." },
      key_points: { type: "array", items: { type: "string" }, description: "Points factuels marquants, un par ligne, classés par pertinence." },
      sources: { type: "array", items: { type: "object", properties: { text: { type: "string" }, url: { type: "string" } }, required: ["text"] } },
      mentioned_people: {
        type: "array",
        description: "Autres personnes citées dans l'actualité en lien avec cette personne (co-dirigeants, partenaires, opposants politiques, etc.) — utile pour repérer des connexions à ajouter.",
        items: { type: "object", properties: { name: { type: "string" }, reason: { type: "string", description: "Pourquoi cette personne est liée à l'actualité trouvée." } }, required: ["name", "reason"] },
      },
    },
...
async function runMonitoringResearch(contact, allContacts) {
  const fullName = ((contact.first_name || "") + " " + (contact.last_name || "")).trim();
  const criteria = (contact.monitoring_criteria && contact.monitoring_criteria.length) ? contact.monitoring_criteria : MONITORING_CRITERIA;
  const context = [
    contact.role ? "poste: " + contact.role : "",
    contact.company ? "entreprise: " + contact.company : "",
    contact.country ? "pays: " + contact.country : (contact.location_city ? "ville: " + contact.location_city : ""),
  ].filter(Boolean).join(", ");

  const searchPrompt =
    "Cherche l'actualité RÉCENTE (idéalement des 7 à 14 derniers jours, sinon la plus récente disponible) concernant : " +
    fullName + (context ? " (" + context + ")" : "") + ". " +
    "Concentre-toi UNIQUEMENT sur ce qui relève de ces critères : " + criteria.join(", ") + ". " +
    "Cherche par nom, titre/poste, entreprise et pays pour bien identifier la bonne personne (attention aux homonymes). " +
    "Si tu ne trouves rien de récent ou de pertinent, dis-le clairement plutôt que d'inventer ou de remonter des infos anciennes non pertinentes.";
  ...
  if (!searchRes.ok) throw new Error("Claude web search error: " + (await searchRes.text()));
  ...
  const structRes = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      model: "claude-sonnet-5",
      max_tokens: 1200,
      system: "Structure ce texte de recherche en rapport de veille exploitable, selon les critères : " + criteria.join(", ") + ". Reste factuel, ne comble aucun vide par de l'invention.",
      messages: [{ role: "user", content: researchText || "Aucun résultat de recherche disponible." }],
      tools: [MONITORING_TOOL],
      tool_choice: { type: "tool", name: "structure_monitoring_report" },
    }),
  });
  if (!structRes.ok) throw new Error("Claude structure monitoring error: " + (await structRes.text()));
  const structData = await structRes.json();
  const toolUse = (structData.content || []).find((b) => b.type === "tool_use");
  const result = toolUse ? toolUse.input : { found_anything: false };

  // Résout les personnes mentionnées contre la base existante — mêmes règles
  // de qualité que partout ailleurs (seuil 0.55, jamais de faux positif silencieux).
  const suggestedContacts = (result.mentioned_people || []).map((mp) => {
    const pool = (allContacts || []).filter((c) => String(c.id) !== String(contact.id));
    const matches = findContactMatches(mp.name, pool);
    const clear = matches.length && matches[0].score >= 0.55 && (matches.length === 1 || matches[0].score - (matches[1] ? matches[1].score : 0) > 0.15);
    return { name: mp.name, reason: mp.reason, matched_contact_id: clear ? matches[0].contact.id : null };
  });
...
function buildPatchFromFields(contact, fields, allContacts) {
  ...
  if (fields.utility_score !== undefined && fields.utility_score !== null && !isNaN(Number(fields.utility_score))) patch.utility_score = Math.max(0, Math.min(10, Math.round(Number(fields.utility_score))));
  if (fields.sentiment_score !== undefined && fields.sentiment_score !== null && !isNaN(Number(fields.sentiment_score))) patch.sentiment_score = Math.max(0, Math.min(10, Math.round(Number(fields.sentiment_score))));
  if (fields.reliability_score !== undefined && fields.reliability_score !== null && !isNaN(Number(fields.reliability_score))) patch.reliability_score = Math.max(0, Math.min(10, Math.round(Number(fields.reliability_score))));
  ...
  if (fields.mentioned_contacts && fields.mentioned_contacts.length && allContacts && allContacts.length) {
    ...
    fields.mentioned_contacts.forEach((mc) => {
      ...
      const isClearMatch = matches.length && matches[0].score >= 0.55 && (matches.length === 1 || matches[0].score - matches[1].score > 0.15);
...
function summarizePatch(patch, allContacts) {
  ...
  const labels = {
    ...
    utility_score: "Utilité", sentiment_score: "Sentiment", reliability_score: "Fiabilité",
  };
...
async function processImageMessage(base64Data, mediaType, captionText, channel) {
  ...
  try {
    ...
    if (matches.length && matches[0].score >= 0.55 && (matches.length === 1 || matches[0].score - (matches[1] ? matches[1].score : 0) > 0.15)) {
...
async function processMessage({ text, channel }) {
  ...
  if (prepNames) {
    ...
    for (const n of prepNames) {
      ...
      const strong = matches.filter((m) => m.score >= 0.55);
      if (strong.length === 0) {
        notFound.push(n);
      } else if (strong.length === 1 || strong[0].score - (strong[1] ? strong[1].score : 0) > 0.15) {
        resolved.push(strong[0].contact);
      } else {
        ambiguous.push({ query: n, candidates: strong.slice(0, 3) });
      }
    }
  ...
  if (pending) {
    if (pending.type === "interview") {
      if (isStopCommand(text)) {
        ...
        if (pending.postSaveOnly) {
          // Suivi ciblé après création/modification : on reste UNIQUEMENT sur ce
          // contact, on ne dérive jamais vers un autre.
          const updatedContacts = await fetchAllContacts();
          const freshContact = updatedContacts.find((c) => String(c.id) === String(pending.contact_id));
          const nextFollowUp = freshContact ? pickPostSaveFollowUp(freshContact, false) : null;
          if (nextFollowUp) {
            await setBotState({ pending: { type: "interview", contact_id: pending.contact_id, field: nextFollowUp.field, question: nextFollowUp.question, postSaveOnly: true } });
            reply += nextFollowUp.question;
          } else {
            await setBotState({ pending: null });
            reply += "C'est complet pour cette fiche, merci !";
          }
        } else if (state.interview_active) {
      ...
      if (isAffirmative(text)) {
        ...
        const baseContact = contacts.find((c) => String(c.id) === String(pending.contact_id));
        const updatedContact = { ...baseContact, ...pending.patch };
        const followUp = pickPostSaveFollowUp(updatedContact, false);
        if (followUp) {
          await setBotState({ pending: { type: "interview", contact_id: pending.contact_id, field: followUp.field, question: followUp.question, postSaveOnly: true } });
      ...
      if (isAffirmative(text)) {
        ...
        const followUp = pickPostSaveFollowUp(created, true);
        if (followUp) {
          await setBotState({ pending: { type: "interview", contact_id: created.id, field: followUp.field, question: followUp.question, postSaveOnly: true } });
  ...
  if (extracted.intent === "question") {
    ...
    const strongMatches = matches.filter((m) => m.score >= 0.55);
    if (strongMatches.length === 0) {
    ...
    } else if (strongMatches.length === 1 || strongMatches[0].score - (strongMatches[1] ? strongMatches[1].score : 0) > 0.2) {
      const contact = strongMatches[0].contact;
      const patch = buildPatchFromFields(contact, extracted.fields || {}, contacts);
      if (Object.keys(patch).length === 0) {
      ...
      const top = strongMatches.slice(0, 4);
